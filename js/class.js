...
$(document).on('localStorageReady', function(){
	...
	if(store.enabled){
		...
		caseresources = store.get('resources').cases;
	} else {
		resources = localStorageBackup.resources.gizmos;
		caseresources = localStorageBackup.resources.cases;
	}
	...
	var options = {
		...
		content: function(){
			...
			var gizmos = el.gizmoHelper.getGizmosByID(resourceIDs, resources, caseresources);
...
function freemiumClassInitialization(){
	...
	function showHideLockedGizmos(){
		$('.locked-gizmo-tag, .locked-case-tag').parents('.card-gizmo').toggle();
