...
$(document).ready(function(){
	...
	function wireSearch() {
      ...
      // Note that we are using a modified version of the typeahead library. 
      // Re: GIZ-10498, JLG 09/19
			...
			var gizmoSuggestionTemplates = {
				...
				suggestion: function(data) {
					...
					if (data.resourceTypeName == 'InteractiveCase') {
						...
						labels += '<span class="labels"><span class="tag-handbook">Handbook</span></span>';
						thumbName = 'TNg';
						...
						if(data.flag == 'N') labels += '<span style="background:green;" class="tag-case">New';
						else{
							labels += '<span class="tag-case">';
							if(data.flag == 'U') labels += 'Updated';
							else labels += 'Featured';
						}
					...
					return '<div data-id="'+data.resourceID+'" ' + caseData +  ' class="gizmo-suggestion">'+labels+'<img src="'+elPaths.imageRoot+'/img/'+imageString+'.png" srcset="'+elPaths.imageRoot+'/img/'+imageString+'@2x.png 2x" class="thumb">'
					 + '<span class="title">'+data.title+'</span></div>';
				}
			};
		...
		if(store.get('resources')===null || store.get('browseTree')===null || store.get('browseStrands')===null || store.get('gizmoReplacements')===null || localStorageOutOfDate('resources')){
			...
			$.ajax({
				...
				$.each(msg.data, function(index,object){
					...
					if (searchEntry.interactivecaseid != 0)
					{
						switch (object.academiclevel) {
							case 'H':
								searchEntry.academicLevel = "High School";
								break;
							case 'A': 
								searchEntry.academicLevel = "AP";
								break;
							case 'M':
								searchEntry.academicLevel = "Middle School";
								break;
						}
					}
				...
				$.ajax({
					method: "GET",
					url: el.getContentRoot() + "/json/qryInteractiveCases.json",
					dataType: 'JSON'
				}).success(function(msg) {
					var casesArray = [];
					$.each(msg.data, function(index,object){
						var casesEntry = {
							interactivecaseid : Number(object.id),
							title : object.title,
							keywords : object.keywords,
							introText : object.introtext
						};
						casesArray.push(casesEntry);
					});
				
					var resources = {'gizmos': searchArray, 'cases': casesArray, 'version': serverLocalStorageVersion};
					if(store.enabled){
					...
					buildBrowseTreeObject();
					
				}).fail(function(request, status, error){
					// qryInteractiveCases AJAX Failure
	...
	function buildBrowseTreeObject(){
		console.log('buildBrowseTreeObject');
	...
	$(document.body).on('click', '.addToClass', function(e) {
		...
		var clicked = $(this),
			...
			caseType = clicked.parents('ul').siblings('a').data('ctype'),
			caseID = clicked.parents('ul').siblings('a').data('cid'),
		...
		var thumbnail = (window.devicePixelRatio > 1) ? elPaths.imageRoot + '/img/' + imageString + '@2x.png' : elPaths.imageRoot + '/img/' + imageString + '.png';
		var notifyArgs = {
			icon: thumbnail,
	...
	$('#heatmapTour').on("hide.bs.modal", function() {
		if($('#dismissHeatmapModal').prop('checked') == true){
			dismissAlert($('#heatmapTour').data('alertid'), elUser.userID);
		}
	});

	$('#nextTour').click(function() {
		$('#heatmapTour1, #nextTour').addClass('hidden');
		$('#heatmapTour2, #closeTour').removeClass('hidden');
	});
	$('#closeTour').click(function() {
		$('#heatmapTour1, #nextTour').removeClass('hidden');
		$('#heatmapTour2, #closeTour').addClass('hidden');
  });
  

  //Promo Trial Modal shown to upgrade eligible users during trial period 

	$('.promo-trial-modal').on("hide.bs.modal", function() {
		if($('#declineUpgradeCheck').prop('checked') === false){
			dismissAlert($('.promo-trial-modal').data('alertid'), elUser.userID);
		}
	});
  ...
  // This should not be used on forms that use jQuery validator, as this will disable
  // the form button even if the form is not submitted (due to validation errors)
  ...
  // Stem Case Library Page
  $('.card-case-large').on('click', function(event){
    window.location.href = elPaths.myself + 'cResource.dspDetail&interactivecaseID=' + $(this).data('caseid');
  })
...
function initializeFreemiumFeatures(){
	...
	function launchGizmoModal(thisGizmo, openModal){
		...
		thisModal.find('#gizmoModalTitle').html(gizmoTitle + gizmoTag);
		thisModal.find('.tag-case, .tag-handbook').addClass('case-header-tag');
		...
		var caseModalLink = thisModal.find('#caseModalLink');

		var launchLink = thisModal.find('.launch-link').data('href') + gizmoID;
		thisModal.find('.launch-link').attr('href', launchLink);

		var lessonLink = thisModal.find('.lesson-link').data('href') + gizmoID;
		thisModal.find('.lesson-link').attr('href', lessonLink);

		if (caseID) {
			var gizmoImage = thisModal.find('.img-responsive').data('src')+"CaseSnap/"+caseID+"DET_1.jpg";
			thisModal.find('.img-responsive').attr('src', gizmoImage);
			thisModal.find('#addToClassButton, .launch-link').hide();
			thisModal.find('.lesson-link, #caseModalLink').show();
		}
		else {
			var gizmoImage = thisModal.find('.img-responsive').data('src')+"GizmoSnap/"+gizmoID+"DET.jpg";
			thisModal.find('.img-responsive').attr('src', gizmoImage);
			thisModal.find('#addToClassButton').data('id', gizmoID);
			thisModal.find('#addToClassButton, .launch-link').show();
			thisModal.find('.lesson-link, #caseModalLink').hide();
		}
    ...
    submitHandler: function(form, e) {
      ...
      // don't actually submit the form, make AJAX call if valid
      e.preventDefault();

      var formphone = $("input[name=phone]").val();
      var formtitle = $("select[name=title] option:selected").text();

      $.ajax({
        type: 'POST',
        url: 'index.cfm',
        data: {
          method: 'cFreemium.actPromoPeriodUpgrade',
          title: formtitle,
          phone: formphone
        },
        success: function(data) {
          window.location.href = elPaths.myself + 'cFreemium.dspPromoAccountComplete'
        }
      });
    },
		rules: {
	...
	$('.promo-upgrade').one('click', function(){
		$('#promoForm').submit();
	});
...
function initializeTeacherHomepage(){
  ...
  // GIZ-10526. Popover removed in favor of link.
...
function getCaseDescription(caseID, caseLibrary){
	var text='';
	$.each(caseLibrary, function(){
		if (this.interactivecaseid == caseID){
			text = this.introText;
		}
	});
	return text;
...
el = {
	gizmoHelper : {
		displayGizmos : function(gizmos, parentElement, templateID, sortProperty){
			try {
				...
				if(typeof gizmos[0] === 'number' || typeof gizmos[0] === 'string'){
					gizmosToDisplay = this.getGizmosByID(gizmos, store.get('resources').gizmos, store.get('resources').cases);
					...
					if(store.enabled){
						gizmosToDisplay = this.getGizmosByID(gizmoIDs, store.get('resources').gizmos, store.get('resources').cases);
					} else {
						gizmosToDisplay = this.getGizmosByID(gizmoIDs, localStorageBackup.resources.gizmos, localStorageBackup.resources.cases);
					}
					for(var i in gizmos){
				...
				// We need to limit the results to one card per Case resource type (e.g. one Case and one Handbook)
				gizmosToDisplay = this.filterCaseDupes(gizmosToDisplay);
		...
		filterCaseDupes : function(gizmosToDisplay){ 
			var cases = [];
			for (var x = gizmosToDisplay.length - 1; x >= 0; --x){
				if (gizmosToDisplay[x].interactivecaseid) {
					var caseIDandType = gizmosToDisplay[x].interactivecaseid + gizmosToDisplay[x].resourceTypeName;
					if($.inArray(caseIDandType, cases)>=0){
						gizmosToDisplay.splice(x, 1)
					}
					else cases.push(caseIDandType);
				}
			}
			return gizmosToDisplay;
		},
		getGizmosByID : function(gizmoIDs, gizmoLibrary, caseLibrary){
			var gizmos = [];
			$.each(gizmoLibrary, function(){
				if($.inArray(this.resourceID, gizmoIDs)>=0){
					this.isTypeCase = this.resourceTypeName == "InteractiveCase" ? 1 : 0;
					if (this.interactivecaseid) {
						this.caseDescription = getCaseDescription(this.interactivecaseid, caseLibrary);
					}
					gizmos.push(this);
				}
			});
	...
	originalElement.on('keyup paste', function(e){
		...
		setTimeout(function() {
			...
			if(currentLength > maxChars){
				element.val(element.val().substring(0,maxChars));
			}
